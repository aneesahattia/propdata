...
<?php
    while ($row = mysqli_fetch_array($result) ) {
        echo '<tr>';
        echo '<td style="text-align: center;" class="small"><img style="border-radius:5px;height:40px;" src="'.HTTP.'uploads/'.$row['profilePicture'].'" alt="'.$row['logger'].'" title="'.$row['logger'].'"><br/>'.$row['logger'].'</td><td onclick="window.location.href=\''.HTTP.'jobs/job.php?jobNo='.$row['timesheetJobNo'].'\';" class="small" style="cursor:pointer;">#'.str_pad($row['timesheetJobNo'],6,"0",STR_PAD_LEFT).'</td><td class="small">'.$row['taskName'].'<br/>'.$row['timetaskComment'].'</td><td>'.$row['timelogged'].'</td>';
        echo '</tr>';
...
?>
    ...
    while ($row = mysqli_fetch_array($result) ) {
        ...
        echo '<td style="text-align: center;" class="small"><img style="border-radius:5px;height:40px;" src="'.HTTP.'uploads/'.$row['profilePicture'].'" alt="'.$row['assigneeName'].'" title="'.$row['assigneeName'].'"><br/>'.$row['assigneeName'].'</td><td class="small"><span style="color:#cc0000">#'.str_pad($row['jobNo'],6,"0",STR_PAD_LEFT).'</span></td><td class="small"><span style="color:#cc0000">No time has been logged for this project.</span></td><td><span style="color:#cc0000">0</span></td>';
        echo '</tr>';
        */
    }
        ?>
